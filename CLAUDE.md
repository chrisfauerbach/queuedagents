...
- **Frontend routing**: React Router with two pages — `DashboardPage` (/) and `JobDetailPage` (/jobs/:id).
...
SQLite at `./data/queue.db`. Two tables:
- `jobs` — Job queue with status tracking, token counts, generation time
